...
class Apriori(object):
    ...
    def __init__(self,use_db=False,query=None,use_iter=False,name='association'):

        '''

        :param use_db: flag to use db as input or file

        :param query: query for selecting the data. should be of the form (select item_array from table)
            ...
            for item in value:
                ...
                for element in _subsets:
                    ...
                    if len(remain) > 0:
                        ...
                        if support_element>0:

                            confidence = getSupport(item)/support_element

                            if confidence >= minConfidence:

                                support_remain = getSupport(remain)

                                if support_remain>0:

                                    lift = confidence/support_remain
    ...
    def save_results_postgres(self,items,rules):
        ...
        self.cursor.execute('drop table if exists '+self.name+'_apriori_supports')

        self.cursor.execute('drop table if exists '+self.name+'_apriori_rules')

        self.cursor.execute('create table '+self.name+'_apriori_supports (items TEXT[],support float)')

        self.cursor.execute('create table '+self.name+'_apriori_rules (items TEXT[],associated_items TEXT[],confidence float,lift float)')
        ...
        for chuk in chunk(items,10000):
            ...
            insert_query = 'insert into '+self.name+'_apriori_supports (items,support) values {0}'.format(records_list_template)
        ...
        for chuk in chunk(rules,10000):
            ...
            insert_query = 'insert into '+self.name+'_apriori_rules (items,associated_items,confidence,lift) values {0}'.format(records_list_template)

            self.cursor.execute(insert_query,chuk)

            self.con.commit()

        self.cursor.execute('CREATE INDEX on '+self.name+'_apriori_supports USING GIN (items)')

        self.cursor.execute('CREATE INDEX on '+self.name+'_apriori_rules USING GIN (items,associated_items)')
    ...
    def save_results_file(self,items,rules):
        ...
        with open(self.name+'_items.pkl','w') as f:

            pickle.dump(items,f)

        with open(self.name+'_rules.pkl','w') as f:
...
if __name__ == "__main__":
    ...
    optparser.add_option('-n', '--name',
                         ...
                         help='name for the rule set (tables will be named accordingly)',
    ...
    apriori_class = Apriori(use_db=use_db,query=inFile,use_iter=True,name=name)
    ...
    if use_db:
        ...
        apriori_class.save_results_postgres(items,rules)

    else:

        apriori_class.save_results_file(items,rules)
